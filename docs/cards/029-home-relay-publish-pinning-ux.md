...
- [ ] Node can create an owner-signed `PinRequest` for locally published content without exposing private key material.
- [ ] HTTP API exposes a local "pin this published CID to my home relay" operation.
- [ ] CLI can publish and pin to the configured home relay, or pin an existing published CID.
- [ ] Dashboard exposes a simple pin-to-home-relay action for recently published content.
- [ ] Dashboard shows clear failure states when no home relay is configured, the relay is unreachable, the relay is not pin-capable, or the pin request is rejected.
- [ ] Successful pinning pins both the content and signed update-log state on the relay.
- [ ] A local manual demo can perform: Alice publish -> Alice pin to relay -> Alice offline -> Bob fetch by `.jolt`.
- [ ] Tests cover signed request creation, API behavior, and the publish/pin workflow without testing dashboard scaffolding.
