...
impl FromStr for IdentityId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.is_empty() || s.len() > 63 || s != s.to_ascii_lowercase() {
            return Err(DwebError::InvalidIdentityAddress(
        ...
        let decoded = BASE32_NOPAD
            ...
            .map_err(|e| DwebError::InvalidIdentityAddress(e.to_string()))?;
        let public_key: [u8; IDENTITY_PUBLIC_KEY_LEN] = decoded.try_into().map_err(|_| {
            DwebError::InvalidIdentityAddress("identity must decode to a 32-byte key".to_string())
        })?;
...
impl JoltAddress {
    pub fn new(identity: IdentityId, path: impl AsRef<str>) -> Result<Self, DwebError> {
        Ok(Self {
            identity,
...
impl FromStr for JoltAddress {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let identity_label = host.strip_suffix(JOLT_SUFFIX).ok_or_else(|| {
            DwebError::InvalidIdentityAddress("address must end with .jolt".to_string())
        })?;
...
fn normalize_path(path: &str) -> Result<String, DwebError> {
