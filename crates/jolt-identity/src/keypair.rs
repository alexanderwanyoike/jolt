...
use jolt_core::{IdentityId, JoltAddress};
use rand::rngs::OsRng;
...
impl NodeIdentity {
    ...
    pub fn load(dir: &Path) -> Result<Self, IdentityError> {
        let key_path = dir.join("keypair.bin");
        if !key_path.exists() {
            return Err(IdentityError::KeyNotFound(key_path.display().to_string()));
        }
    ...
    pub fn to_libp2p_keypair(&self) -> libp2p::identity::Keypair {
        ...
        let ed25519_keypair = libp2p::identity::ed25519::Keypair::try_from_bytes(&mut full_bytes)
...
mod tests {
    ...
    fn sign_fails_verification_with_wrong_data() {
        ...
        let result = crate::signing::verify_signature(
