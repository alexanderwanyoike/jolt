...
//! Run with: cargo test -p jolt-network --test nat_traversal
...
use jolt_network::{DaemonHandle, NetworkConfig, NetworkNode};
...
/// Test F: Two jolt nodes on the same LAN, content transfer via direct dial.
...
#[tokio::test(flavor = "current_thread")]
#[ignore = "manual patchbay test: requires Linux network namespace support"]
async fn test_lan_jolt_content_transfer() -> Result<()> {
    init_patchbay();
...
/// Test G: Three jolt nodes on the same public network. Publisher publishes content,
