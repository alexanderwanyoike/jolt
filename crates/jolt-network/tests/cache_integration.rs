...
async fn three_node_cache_relay() {
    ...
    assert_eq!(
        response_b.data, test_data,
        "B should get correct data from A"
    );
    ...
    let mut node_c = NetworkNode::new_tcp(identity_c, store_c, no_mdns_config()).unwrap();
    ...
    assert_eq!(
        response_c.data, test_data,
        "C should get correct data from B's cache"
    );
    ...
    assert!(
        sig_valid,
        "original publisher's signature should still verify"
    );
